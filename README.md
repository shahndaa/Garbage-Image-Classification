...
├── app/
│   └── app.py              # Gradio demo (upload a photo → get a prediction)
├── notebooks/
│   └── training_demo.ipynb # Colab-friendly, step-by-step walkthrough
├── assets/                  # training curves & confusion matrix (generated by train.py)
├── models/
│   └── garbage_classifier.keras   # trained model, included so the demo works out of the box
...
Launches a local **Gradio** interface: upload a photo and get the predicted material, a
confidence breakdown across all 6 classes, whether it's recyclable, and a disposal tip.

> 💡 This can also be deployed for free on [Hugging Face Spaces](https://huggingface.co/spaces)
> (Gradio SDK) so anyone can try it from a link, with no installation required — just push
> `app/app.py`, `src/`, `models/`, and `requirements.txt` to a new Space.
...
- [ ] Deploy the Gradio demo to Hugging Face Spaces and link it here
